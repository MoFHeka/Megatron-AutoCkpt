...
def load_checkpoint_for_megatron(
...
def _clean_up_storage_for_megatron(max_ckpt_num):
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        ...
        if os.path.exists(args.save):
            ckpt_folders = [
                folder for folder in os.listdir(args.save) if folder.startswith("iter_")
            ]
...
class InMemCkptMegatron(InMemoryCheckpointManager):
    ...
    def _save_to_storage_specific(self):
        ...
        if get_checkpoint_names_fun_exists is True:
            model_checkpoint_name, optim_checkpoint_name = get_checkpoint_names(
                args.save, iteration, args.no_load_optim, args.use_distributed_optimizer
            )
        else:
        ...
        logger.info(
            "Successfully saved checkpoints to storage at iteration {:7d} to {}".format(
                iteration, args.save
            )
        )
    ...
    def save_in_memory_checkpoint_for_megatron(
        self, iteration, model, optimizer, opt_param_scheduler
    ):
...
def initialize_autockpt_for_megatron(
    save_mem_interval, save_storage_interval, max_ckpt_num
):
