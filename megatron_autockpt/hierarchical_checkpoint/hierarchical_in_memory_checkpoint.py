...
class InMemoryCheckpointManager(ABC):
  ...
  def __init__(self):
    ...
    self._sync_save_to_storage_group = torch.distributed.new_group(
        backend="gloo")
    self._sync_save_to_storage_thread = threading.Thread(
        target=self._sync_save_to_storage, daemon=True)
    self._sync_save_to_storage_thread.start()
    self._sub_process = multiprocessing.Process(
        target=self._on_demand_save_to_storage, daemon=True)
  ...
  def _make_pin_memory_buffer(self, kwargs):
    ...
    for state_dict_name, state_dict_value in kwargs.items():
      if state_dict_value:
        self._pin_memory_buffer[state_dict_name] = self._make_state_dict_buffer(
            state_dict_value)
  ...
  def _sync_save_to_storage(self):
    ...
    while self._finish_save_to_storage_event.wait():
      ...
      try:
        ...
        logger.info(
            'Exit exception when do barrier in _sync_save_to_storage: %s', e)
  ...
  def _write_to_pin_memory(self, iteration, kwargs):
    ...
    if torch.distributed.is_initialized():
      try:
        torch.distributed.all_reduce(self._save_to_storage_done,
                                     op=torch.distributed.ReduceOp.SUM)
      except Exception as e:  # pylint: disable=broad-except
        logger.info(
            'Exit exception when do all_reduce in write_to_pin_memory: %s', e)
    if self._save_to_storage_done.item() > 0:
      logger.info(
          'Still save checkpoints to storage, skip write_to_pin_memory.')
      ...
      logger.info('Start to save in memory checkpoint at iteration %d .' %
                  (iteration))
      for state_dict_name, state_dict_value in kwargs.items():
        if state_dict_value and state_dict_name in self._pin_memory_buffer:
          self._copy_state_to_buffer(state_dict_value,
                                     self._pin_memory_buffer[state_dict_name])
      ...
      logger.info('Successfully saved in memory checkpoint at iteration %d .' %
                  (iteration))
      ...
      if self._share_memory and \
       ...
       self._share_memory["iteration"] not in self._saved_iterations:
        with self._memory_buffer_lock:
          logger.info('Start to save checkpoints to storage %d .' %
                      (int(self._share_memory["iteration"])))
